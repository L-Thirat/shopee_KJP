...
for index, row in df.iterrows():
    ...
    #["mdw darul ulum kangkung", {"entities": [[0, 14, "POI"]]}]
    ...
    #         start += 1
    #         end += 1
    if end_poi < (len(raw_address)):
        if raw_address[end_poi] != " ":
            raw_address = raw_address[:end_poi] + " " + raw_address[end_poi:]
    if start_poi > 0:
        if raw_address[start_poi-1] != " ":
            raw_address = raw_address[:start_poi] + " " + raw_address[start_poi:]
            start_poi += 1
            end_poi += 1
            print(start_poi, end_poi, raw_address)
